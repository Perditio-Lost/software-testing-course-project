...
const load = () => {
  request.get('/teacher/selectPage', {
    ...
    if (res.code === '200') {
      ...
      // 加载每个教师的头像
      data.tableData.forEach(teacher => {
        loadTeacherAvatar(teacher)
      })
    }
  })
}
...
// 加载教师头像
const loadTeacherAvatar = (teacher) => {
  if (!teacher.id) return
  request.get('/teacher/getAvatarUrl/' + teacher.id).then(res => {
    if (res.code === '200' && res.data) {
      teacher.avatarUrl = res.data + '?t=' + Date.now()
    }
  })
}
...
const handleEdit = (row) => {
  ...
  // 加载头像
  if (data.form.id) {
    request.get('/teacher/getAvatarUrl/' + data.form.id).then(res => {
      if (res.code === '200' && res.data) {
...
const uploadAvatar = (id) => {
  return new Promise((resolve, reject) => {
    ...
    request.post('/teacher/uploadAvatar', formData).then(res => {
      if (res.code === '200') {
        resolve()
      } else {
